...
<div class="container spark-screen">
    <div class="row">
		...
		{{-- {!! Form::open(array('method' => 'GET','route' => 'teachersetup.search')) !!}  
	 ...
	 --}}
        ...
        <div class="col-md-10 col-md-offset-1">
			...
			@endif
				{{-- <p>
					<a class="btn btn-primary" href="{{route('teachersetup.create')}}" >Add new data</a>
					<a class="btn btn-default" href="{{ route('teachersetup.index') }}" role="button">Show All</a>
				...
				<div class="row">
					...
					<div class="col-md-8">
						...
						<a style="background-color:#00663d; color:#fff;" class="btn btn-primary" href="{{route('teachersetup.create')}}" >Add New Teacher</a>
						<a style="background-color:#00663d; color:#fff;" class="btn btn-primary" href="{{ route('teachersetup.index') }}" role="button">Refresh</a>
						<!-- <a class="btn btn-info" href="{{ url('parents') }}" role="button">Transfer</a> -->
					</div>
					...
					<div class="col-md-4">
			            ...
			            {!! Form::text('searchString', null, array('class' => 'inputForm',"style"=>" font-size:18px;",'placeholder'=>'Type Teacher Name')) !!}
			...
			@if (count($rows))
	            ...
	            <form action="{{ route('categories6.deleteMultiple') }}" method="post">
                {{ csrf_field() }}
                <thead>
                    <tr>
                      <th style="width: 8px;">
	            <div class="panel panel-primary">
	               {{--  <div style="background-color:#00663d; color:#fff; padding-top: 12px;padding-bottom: 12px;padding-left: 20px;"  class="panel-primary2">List of Teachers</div> --}}
	                  	...
	                  	<p>
                           <div class="col-sm-12"> 
							...
							<tr>
								...
								<td><input type="checkbox" name="categories6[]" class="checkboxes" value="{{ $row->TeachersetupID }}" /></td>
                            <td>{{$row->TeacherSetupName or 'DEFAULT'}}</td>
                               ...
                               <!--  <td>{{$row->ClassName or 'DEFAULT'}}</td>
								...
								<td><a class="btn btn-xs btn-success" href="{{ route('teachersetup.show',[$row->TeachersetupID])  }}">Show
		                                                  {{--  <i class="glyphicon glyphicon-eye-open"> --}}</i></a> </td>   
								<td>
		                        
			                         <a href="{{ action('TeachersetupController@edit', array($row->TeachersetupID)) }}"
